...
class NLP_model_index(nn.Module):
    def __init__(self):
        ...
        # self.index_fc = nn.Linear(model_config.hidden_size, 2)  # FUNNEL-TRANSFORMER
    ...
    def forward(self, x):
        ...
        # index_logits = torch.mean(torch.stack([self.index_fc(self.high_dropout(lhs)) for _ in range(5)], dim=0), dim=0)
...
if __name__ == '__main__':
    ...
    csv_split_output = read_csv_split(csv_path)
    index_texts_wt, index_indices = csv_split_output['index_texts_wt'], csv_split_output['index_indices']
    ...
    # indices = [(train_indices, val_indices) for (train_indices, val_indices) in kf.split(np.arange(len(csv_dataset)))]
    # train_indices, val_indices = indices[FOLD]

    # train_dataset, val_dataset = Subset(csv_dataset, train_indices), Subset(csv_dataset, val_indices)
    train_dataset, val_dataset = train_test_split(csv_dataset, test_size=0.2)
