...
if __name__ == '__main__':
    ...
    # wt
    q_texts = tokenizer(q_texts_wt, is_split_into_words=True, padding=True, truncation=True, return_tensors='pt').to(device)
    r_texts = tokenizer(r_texts_wt, is_split_into_words=True, padding=True, truncation=True, return_tensors='pt').to(device)
    ...
    with open(f'./index_result_{Path(PATH).stem}.csv', 'w', newline='', encoding="utf-8") as csv_file:
        ...
        for index, (id, q_prime, r_prime) in enumerate(zip(ids_ans, q_prime_index_ans, r_prime_index_ans)):
            # add " " back
